...
/**
 * 
 * I am not proud of this library...
 * This will be revamped soon hopefully!
 */
...
class Holder {
  constructor(name, mouseEnter, mouseExit) {
    ...
    this.toggle.className = 'toggle_node';
    this.toggle.onmousedown = () => {
      this.collapse();
    }
    this.ele.append(this.toggle);
    ...
    this.collapsed ? this.toggle.innerText = "+" : this.toggle.innerText = "-"
    this.collapsed ? this.toggle.style.setProperty('background-color', "rgba(33, 150, 243, 0.5)") : this.toggle.style.setProperty('background-color', "rgba(244, 133, 129, 0.5)")
  }
...
class Slider {
  ...
  changeVal(newVal) {
    ...
    this.elementRight.innerText = this.val.toFixed(this.fix)
  }
  ...
  update(genome = this.genome) {
    ...
    for (let l = 0; l < genome.layers; l++) {
      ...
      for (let n = 0; n < layer.length; n++) {
        positions[layer[n].id] = { x: (dx * l) + (dx / 2), y: (dy * n) + (dy / 2) + (10), bias: layer[n].bias, layer: layer[n].layer, val: layer[n].val, name: layer[n].name }
      }
    }
    ...
    for (let i = 0; i < genome.nodes.length; i++) {
      ...
      if (positions[genome.nodes[i].id].name != "Neuron") {
        this.p5.text(positions[genome.nodes[i].id].name, x, y-(30/2))
      }
      this.p5.fill(0, 255);
      this.p5.stroke(0, 0)
...
const createEl = (el, content, classList = [], isContentElement = false, onclick = () => {}, item) => {
  var element = document.createElement(el.toUpperCase());
  ...
  if (content) {
    if (isContentElement) element.appendChild(content);
      ...
      if (item.org != undefined) {
        if (item.org.alive == true) {
          element.style.background = "rgba(0, 255, 0)"
        }else {
          element.style.background = "rgba(255, 0, 0)"
        }
      }
    };
  }
...
const createTree = (parent, arr) => {
  arr.forEach(item => {
      const li = createEl('li', createEl('div', item.text, ['tf-nc'], false, item.onclick, item), [], true);
...
class Tree {
  constructor(name, obj) {
    ...
    let title = new Label(this.name, "").ele
    this.ele.append(title)
    this.ele.classList.add("tree")

    drawTree(this.ele, this.obj);
    ...
    slider.addEventListener('mousemove', (e) => {
      ...
      const walky = (y - startY) * 1.5; //scroll-fast
